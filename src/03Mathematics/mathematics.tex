...
Linear operators transform an input vectors into an output vector. For some operators such transformation is always \emph{reversible}: There exist another operator that can "cancel" the action of the first, as illustrated in Figure \ref{fig:dualOperators}(a), where the operator $\op{U}$ acts as the \emph{inverse} of the operator $\op{T}$:
\[
\op{U}\circ\op{T}=\op{I}\,.
\]
...
Scalar product leads to more sophisticated relations between operators. One such relation, shown in Figure \ref{fig:dualOperators}(b), amounts to one operator being "inverse relative to the second input" of the binary scalar product operator $\lbrack\cdot\rbrack$. This can be expressed in the following equation:
\[
\lbrack\cdot\rbrack\,\ket{a}\,\ket{b}=\lbrack\cdot\rbrack\,(\op{T}\ket{a})\,(\op{U}\ket{b})\,.
\]
In a more familiar infix notation, this becomes
\[
...
One more relation we consider is also generated by the scalar product. As illustrated in Figure \ref{fig:dualOperators}(c), the relation means that one operator (e.g. $\op{T}$) achieves the same affect as the other (e.g. $\op{U}$), but acting on the second input of the binary scalar product operator $\lbrack\cdot\rbrack$. Symbollically we can write:
\[
\ket{a}(\op{U}\cdot\ket{b})=(\op{T}\ket{a})\cdot\ket{b}\,.
\]
...
\[
\ket{a}(\op{S}_x\cdot\ket{b})=(\op{S}_x\ket{a})\cdot\ket{b}\,.
\]
...
\begin{exercise}
	Find the adjoint of the rotation operator $\op{R}_\theta$.
\end{exercise}

Finally, combining the notion of the \emph{dual} vector space and the \emph{scalar equivalent} operators, we arrive at an important concept of the \emph{adjoint operator}.
\begin{figure}[htbp]
	\centering
...
\end{figure}
First, recall that each vector $\ket{a}$ has dual $\bra{a}$, and the whole space of ket-vectors has a dual counterpart, as shown in Figure \ref{fig:adjointOperator}. Given an operator $\op{T}$ acting on ket-space, we can define an operator acting on bra-space. The rule is simple, to find the action $\op{T}^*\bra{a}$ we first find $\op{T}\ket{a}=\ket{c}$ and then find its dual $\bra{c}$, by partially applying scalar product:
\[
 \bra{c}=(\op{T}\ket{a})\cdot\tus\,.
\]
Let's examine how this bra-vector $\bra{c}$ acts on an arbitrary ket-vector $\ket{b}$:
\[
\braket{c}{b}=(\op{T}\ket{a})\cdot\ket{b}\,.
\]
If the scalar equivalent for the operator $\op{T}$ exists, we could write:
\[
\braket{c}{b}=\ket{a}\cdot(\op{U}\ket{b})=\bra{a}\op{U}\ket{b}\,.
\]

The adjoint of the operator $\op{U}$ is often denoted using "\emph{dagger}"-notation:
\[
\op{T}=\op{U}^\dagger\,.
\]
