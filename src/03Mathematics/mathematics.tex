...
Two important characteristic of a random series are the \emph{average value} and the \emph{variance}. The former can often be linked to the most likely outcome, while the latter describes how large is the spread of values. Before we give formal definitions to these concepts, let's consider a specific example.

Suppose we have 100 fair dice and toss them all at once. Denoting the sum total of face values as $V$, we can state that it can be anywhere from 100 to 600, although neither of these extreme values is likely. But which value will be most likely?

\subsection{Poissoinian Process}
