...
\lettrine[lines=2]{\color{darkocre}M}{athematical concepts and tools used in quantum theory are} not significantly different from those used in classical physics. This fact, of course, does not make them any easier. However, it is important to remember that \emph{there is no special mathematics that makes quantum physics extra challenging}.
...
\begin{myprereq}{Prerequisite Knowledge}
	...
	\begin{itemize}
		...
		\vspace{-0.5cm}
		\item Basic algebraic operations.
		\item Simple intuition about probabilities.
		\item Plots and axes.
		\item Familiarity with functions like $\sin\, x$, $\log\, x$.
		\item Use of subscripts, as in $x_1$.
	\end{itemize}	
\end{myprereq}

\newpage
\section*{Noncommutativity}
Before we start learning specific mathematical tools, it is essential to understand and appreciate one extremely powerful concept that sets apart advanced mathematics used in quantum physics. This is the concept of \emph{noncommutativity}.
\begin{mydef}{Noncommutativity}
	An operation $\bullet$ is \emph{noncommutative} if its result depends on the order of arguments:
	\[
		A\,\bullet\,B\ne B\,\bullet\,A\,.
	\]
	
	{\it Example}: $x^\wedge y \ne y^\wedge x$.
\end{mydef}
Noncommutativity brings variety and power. It is a natural ingredient of human language where various combinations of same letters have very different meaning: {\bf post $\ne$ stop $\ne$ spot}. Furthermore, our actions and operations are often noncommutative, as we are reminded every time we dress up and go outside (in that order!) Obviously, the result of putting on socks and then shoes differs from doing it in reverse order. In short, noncommutativity is an essential feature of the world and our interactions with it. If mathematics is to adequately describe all that, it must have noncommutativity. 

Usually, first introduction to algebra starts with addition, followed by multiplication. Both operations are \emph{commutative}: $x+y=y+x$ and $x*y=y*x$. Very quickly students learn about subtraction, division, and exponentiation ($x$ to the power of $y$) which lack commutativity. At even higher level, \emph{function composition} (discussed in Section \ref{sec:Numberlikes}) demonstrates the power of noncommutativity. Thus, noncommutativity is present in the "usual" mathematics of classical physics but at a higher level. The situation in quantum physics is fundamentally different!


In quantum theory noncommutativity greets us at the door: even the basic quantities like position $x$ and momentum $p$ are now represented not by numbers, but by \emph{special mathematical objects}, called \emph{operators}, which, while extending and improving the concept of a number (see Fig. \ref{fig:numbersOperators} and section \ref{sec:Numberlikes}), lose the comfortable property of commuting.  To illustrate the point, consider the product $xp$, the physical meaning of which will be discussed later in section \ref{subsec:PhaseSpace}. In quantum theory we have
\[
xp\ne px\,.
\] 
The difference $xp-px$ is closely related to the most fundamental physical quantity called \emph{quantum of action}, discussed in section (XXX??). 
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{numbersOperators}
	\caption{Usual numbers are, in a certain sense, special case of \emph{linear operators} -- powerful mathematical objects explored later in the book.}
	\label{fig:numbersOperators}
\end{figure}

Not only position $x$ and momentum $p$ are "upgraded" from numbers to operators, but any function of them, like their product $xp$, or kinetic energy $E_k=p^2/2m$ or potential energy $E_p=kx^2/2$ and so on\footnote{We will learn how to understand a function of an operator later in the book.}, become operators too. In quantum theory any quantity that can be \emph{observed} (measured) is described by an operator instead of a number. 

\begin{mybio}{Dirac on Quantum versus Classical Mechanics}
	Paul Dirac, in \S 7 of his textbook {\it Principles of Quantum Mechanics} writes
	...
	In mechanics, position $x$ and momentum $p$ are called \emph{dynamical variables}, since their change in time is governed by the \emph{dynamical laws} -- laws that involve forces (\emph{dynamis} -- "force" in Ancient Greek). The rules for manipulation of $x$ and $p$ , including addition and multiplication, are called \emph{algebra}. Classical physics relies on a commutative algebra where $x$ and $p$ are usual numbers and $xp=px$. But classical physics fails to explain quantum phenomena.
\end{mybio}


To summarize: Noncommutative mathematics more accurately captures the intricate quantum features of nature. It is the essential feature of quantum theory where noncommutativity is placed at the very foundation.

\section{Randomness}
Randomness is the opposite of predictability, certainty, and \emph{determinism}. Randomness (or \emph{indeterminism}) plays an essential part in classical and quantum physics.

Mathematical description of randomness is based on the idea of \emph{probability}. Probability quantifies or measures the notion of \emph{likelyhood} of certain outcomes. A common example is the toss of a "fair" coin where the probability of tail is $1/2$. Or take another typical case: an ideal six-sided die. If we throw it 6 million times, and each side is equally likely to appear, we expect to see each value to come up 1 million times. Suppose we are interested in those cases when the face value is divisible by 3. That happes 2 million times, one million for 3 and another for 6. The chances that we will see such an event is therefore 2 out of 6 or 1/3.
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{probabilitySixSidedDie}
	\caption{Results of throwing a real six-sided die.}
	\label{fig:probabilitySixSidedDie}
\end{figure}

\begin{mydef}{Probability as Frequency}
	Probability of an event $E$  can be defined as the \emph{relative frequency} or ratio of the number of times the event occured in a series of trials to the total number $N$ of trials:
	\[
	P_E = \frac{N_E}{N}\,,
	\]
...
Let's consider a real experiment with a six-sided die, illustrated in Figure \ref{fig:probabilitySixSidedDie}. In this experiment a die has been first thrown 200 times, then again 200 times, and finally 1000 times. The histograms in the second column show how many time each face value appeared in a given series of throws. The plots in the first column correpond to the \emph{running value} of the probabillity that the face value is divisable by 3 (i.e. equals 3 or 6) has appeared (event $E_{3|6}$). The running value means that we recalculate the ratio $N_{3|6}/N$ with each throw. This explains large oscillations between 0 and 1 at the very begginning, when we either get or don't get the event $E$ in the first throw.

The histrograms reveal that in relatively short experiments ($N<1000$) different face values might appear with significantly different probabilities, as if the die is not \emph{fair}. This also results in the probability of the event $E_{3|6}$ to differ appreciably from the "ideal" value of $1/3\approx 0.33$, shown as a horizontal line in the plots. It requires a longer series of throws to see that the die is pretty fair and the probability of the event $E_{3|6}$ is close to the ideal.

\begin{myrem}{Irreversibility}
	The throw of a die is a good example of what is required in an actual quantum experiment. When a die is hitting the surface of a table, it might bounce off several times before settling on a certain face. Whatever energy (kinetic of potential) the die had is now \emph{irreversibly} gone. If we had an ideal perfectly elastic materials, the die would endlessly keep bouncing up and down and we would not be able to complete a single observation.
	
	Every observation in physics has a certain \emph{finality}, caused by some \emph{irreversible process} that transfers energy and information to the observing "device" (e.g. camera or human eye and brain). We will be returning to this important point in other chapters.
\end{myrem}

\subsection{Ensembles and Series}
In experiments with random outcomes we must deal with many trials. In each trial we observe a certain value $v_i\,,\,\, i=\overline{1,N}$. The sequence of values $v_1,\,v_2\,,v_3,\,\ldots,\,v_N$ is called an \emph{statistical ensemble} or a \emph{random series}.


\subsection{Average and Variance}
Two important characteristics of a random series are the \emph{average value} and the \emph{variance}. The usefulness of these parameters comes from their ability to concisely describe the main features of a very rich set of random values. Average can often be linked to the most likely outcome, while variance describes how large is the spread of values around the average. 

The meaning of average or expectation value is quite simple. Imagine we throw a big party for $M$ guests. Each guest can eat some number of candies, ranging from 0 to 10. Although we can't know this number in advance for each guest, we can still try to guess how many candies to purchase in order to satisfy all guest. Assuming that each guest "is the same" and can eat $N$ candies, we would buy $MN$ candies (perhaps more to be safe and guarantee that there will be enough candies for all). $N$ is the \emph{average} number of candies per guest. 

Before we give formal definitions to these concepts, let's consider a specific example. 
Suppose we have $M$ fair dice and toss them all at once. Denoting the sum total of face values as $V$, we can state that it can be anywhere from $M$ to $6M$, although neither of these extreme values is likely. But which value will be most likely? The total value $V$ is the sum of face value of each individual die:
\[
V = v_1 + v_2 + v_3 + \ldots + v_{M}\,.
\]
We can group terms according to their value and rewrite $V$  as
\[
V = 1\cdot n_1 + 2\cdot n_2 + 3\cdot n_3 + \ldots + \ldots 6\cdot n_6\,,
\]
...
\[
V = 1\cdot p_1 N + 2\cdot p_2 N + 3\cdot p_3 N + \ldots + \ldots 6\cdot p_6 N = N \avg{v}\,,
\]
where
\[
\avg{v} = 1\cdot p_1 + 2\cdot p_2 + 3\cdot p_3 + \ldots + \ldots 6\cdot p_6 
\]
is called \emph{average} of a face value. Another way to write average is
\[
\avg{v} = \frac{V}{N}\,.
\]
\begin{mydef}{Average Value}
	Given a random series $v_1,\,v_2\,,v_3,\,\ldots,\,v_M$, the average value is defined as
	\[
	\avg{v} = \frac{v_1 + v_2 + v_3 + \ldots + v_{M}}{M}\,.
	\]
	If each outcome $v_k$  can take a value $u_i$ with a probability $p_i$, then the average can be written as
	\[
	\avg{v} = u_1 p_1 + u_2 p_2 + u_2 p_2 + \ldots + u_m p_m\,. 
	\] 
	Along with \emph{average value} the term \emph{expectation value} is often used. 
\end{mydef}
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{probabilityManyDice}
	\caption{Results of throwing $M$ six-sided dice multiple times. The average and variance.}
	\label{fig:probabilityManyDice}
\end{figure}

In Figure \ref{fig:probabilityManyDice} the results of simulations of throwing $M=10,50, 100$ six-sided balanced dice is shown. The horizontal axis corresponds to the total face value $V=v_1+\ldots+v_M$, while the vertical bars are proportional to the probability for a specific value to appear in a series of throws ($N=100000$ in this example).

As immediately seen from the histograms, the extreme values $V_{min}=M$ and $V_{max}=6M$ are stupendously unlikely. The average value $\avg{V}$ in this example lies in the middle between the minimum and maximum and also has the highest probability to appear. There are deviations from the average in each trial, but the distribution of $V$ becomes seemingly narrower with increasing number of dice used. This is a curious feature of many processes that involve several random variables contributing to the final result.

To characterize the deviations from the average we can use \emph{variance}. The idea is simple: Given a random series $V_1,\,V_2\,,V_3,\,\ldots,\,V_N$ we first calculate the average value $\avg{V}$. Then for each value $V_k$ we calculate its deviation from the average $\Delta V_k = V_k - \avg{V}$. Next we want to find what the average of this deviation. But we must be careful, since positive and negative deviations appear equally likely and will cancel each other. To get around this, we first square the deviation and only then find the average $\avg{(\Delta V)^2}$, obtaining \emph{variance}. We can give the following definition:
\begin{mydef}{Variance}
	Given a random series $v_1,\,v_2\,,v_3,\,\ldots,\,v_M$, with the average $\avg{v}$, the variance of the series is the measure of its spread around the average value, given by
	\[
	\textrm{Var}\, v = \avg{\left(v-\avg{v}\right)^2}\,.	
	\]	
\end{mydef}
\begin{exercise}
	Show that 
	\[
	\textrm{Var}\, v = \avg{v^2}-(\avg{v})^2\,,	
	\]	
	where $\avg{v^2}$ is the average value of $v^2$:
	\[
	\avg{v^2} = \frac{v^2_1 + v^2_2 + v^2_3 + \ldots + v^2_{M}}{M}\,.
	\]
\end{exercise}

If $v$ is measured in meters, then the units for variance would be in meters squared. It is often convenient to characterize the spread around the average in the same units as the average. This can be done using \emph{deviation}:
 \begin{mydef}{Deviation}
 	Given a random series $v_1,\,v_2\,,v_3,\,\ldots,\,v_M$, with the average $\avg{v}$ and variance $\textrm{Var}\,v$, the deviation of the series is the measure of its spread around the average value, given by
 	\[
 	\Delta v = \sqrt{\textrm{Var}\,v}\,.
 	\]	
 \end{mydef}
 \begin{remark}
 	\emph{Average} and \emph{deviation} are important and convenient characteristics of a random series. They often (but not always!) concisely capture two basic aspects of the random series of values: the most likely result and the spread around it.
 \end{remark}



\subsection{States And Hidden Variables}\label{subsec:statesHiddenVariables}
To make notation used in quantum physics feel less foreign we will introduce it early and in a more familiar setting. To this end, consider a scenario where a student, let's call him Alex, goes to a vending machine to buy a glass of juice. The machine offers two options: (1) A pure apple juice and (2) a pure orange juice, as shown in Fig. \ref{fig:correlations}.
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{correlations}
	\caption{Alex and Bella choose a juice at a vending machine.}
	\label{fig:correlations}
\end{figure}

Suppose that each day for the whole year Alex chooses option 1. We can conclude almost with certainty that in future observations, Alex will choose option 1. We will say that Alex is in such a \emph{state} that, if confronted with a choice at the vending machine, he will always go with the option 1. Symbolically we will write the \emph{state} of Alex as $\ket{1}$.

Imagine another student, let's name her Bella, who sometimes chooses option 1 and other times -- option 2 in an unpredictable (random) fashion. If both options are chosen equally frequently, we can say that Bella is in an \emph{uncertain state} and write it symbolically as follows: $\frac{1}{2}\ket{1}+\frac{1}{2}\ket{2}$. All this mathematical expression states is our ability to predict the results of future observations, based on the observations performed in the past.

If students make up their minds right at the vending machine, being completely uncommitted prior, then calling the state of Bella \emph{uncertain} is justified. But another situation is possible: Bella approaches the machine already knowing what she wants that day and her mental state is quite certain in each transaction, however it is unknown to an outside observer. In other words, the factors that determine Bella's choice are \emph{hidden} to the outside observer. In quantum theory such factors are called \emph{hidden variables}.

\begin{mybio}{Element Of Reality}
	It is not hard to accept the fact that, in the examples considered above, Bella is undecided before she interacts with the vending machine. But it is harder to imagine that Alex is undecided every time and yet \emph{always} decides in favor of the apple juice. Such a certainty, the complete absence of randomness, leads us to think that there is \emph{something in the reality} that determines Alex's choice.
	
	Albert Einstein, Boris Podolsky, and Nathan Rosen suggested that
	"{\it If, without in any way disturbing a system, we can predict with certainty (i.e., with probability equal to unity) the value of a physical quantity, then there exists an element of physical reality corresponding to this physical quantity. }"
...
\begin{remark}
	When we use the concept of the state, should we understand it as the state of the system, or our the state of "knowledge" about a system?
\end{remark}

\subsection{Joint Measurement}\label{subsec:jointMeasurement}
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{jointMeasurement}
	\caption{Alex and Bella choose a juice at a vending machine.}
	\label{fig:jointMeasurement}
\end{figure}
When a student chooses a drink we consider it an \emph{event}. We can speak about random events (random outcomes), their probabilities, expected values, and deviations. It is also possible to discuss two or more events in order to analyze possible connections between them. Suppose, for example, that Alex and Bella buy their drinks from different vending machines situated far apart, as illustrated in Fig. \ref{fig:jointMeasurement}. Their choices are recorded and later examined, either separately or \emph{jointly} (together).

If we consider Alex and Bella as a single team, then their combined (\emph{joint}) choice can be considered a single event $E$. There are four different potential outcomes for each joint event: (1) Both Alex and Bella choose apple juice, $E=AA$, (2) Alex chooses apple juice, Bella chooses orange juice, $E=AO$, (3) Alex chooses orange juice, Bella -- apple, $E=OA$, and finally (4) both choose orange juice, $E=OO$. 

Imagine we have a long observational record, from which we can calculate relative frequency of each of the four possible outcomes: $AA\to p_1$, $AO\to p_2$, $OA\to p_3$, and $OO\to p_4$.  Similar to the notation used to represent a state of a single person, we can write the general state of the "team" as follows:
\[
p_1\ket{11} + p_2\ket{12} + p_3\ket{21} + p_4\ket{22}\,,
\]
where $\ket{11}$ represents the situation when both Alex and Bell choose option 1 (apple juice) and $p_1$ is the probability of this outcome, $\ket{12}$ represents the situation when Alex chooses option 1 while Bell chooses option 2 with relative frequency $p_2$. Similarly for the remaining terms of the sum. For example, the case when Alex always chooses apple juice, while Bella vacillates between two equally appealing options would be described by the following state of the team (\emph{joint} state):
\[
\ket{T_1} = \frac{1}{2}\ket{11} + \frac{1}{2}\ket{12}\,.
\]
\begin{exercise}
	Suppose the joint state of two students is described by the state
	\[
		\ket{T_0} = \frac{1}{2}\ket{11} + \frac{1}{2}\ket{22}\,.
	\]
	
	What can we say about Alex? What can we say about Bella? How does this state compare to $\ket{T_1}$ given above? Can we distinguish between $\ket{T_0}$ and $\ket{T_1}$ observing Alex alone? Bella alone?
\end{exercise}

\subsection{Correlations}
Analyzing the state $\ket{T_0}$ we conclude that Alex randomly chooses any juice with equal probability. The same can be said about Bella. However, despite both Alex and Bella acting randomly, there is certain connection between their behavior, which results in total coincidence of their choices and complete absence of cases when they choose different juice. This is an example of \emph{correlation} between random processes. 

For the state $\ket{T_0}$ the correlation is very strong, complete, in fact. In other words, given $\ket{T_0}$ and observing only the behavior of Alex, we don't need to observe Bella, since her choices will "mirror" those of Alex as if she had no autonomy at all. If we observed Bella instead, we could predict the choices of Alex. Although random, the behaviors of Alex and Bella are \emph{not independent}, they are \emph{perfectly correlated}.
\begin{exercise}
	Consider the joint state given by
	\[
	\ket{T} = \frac{4}{10}\ket{11}+\frac{1}{10}\ket{12} + \frac{1}{10}\ket{12}+\frac{4}{10}\ket{22}\,.
	\]
	
	What can we say about the behavior of Alex? Of Bella? Is there any correlation in their behavior?
\end{exercise}

It is easy to create a perfect correlation between Alex and Bella if they can communicate. For instance, if they meet and agree to choose the same juice on each day. To make their choice random they flip a coin to determine the type of choice for that day. In this case we would observe the behavior described by the state $\ket{T_0}$. 
...
\[
\ket{T} = \frac{1}{12}\ket{11} + \frac{1}{6}\ket{12} + \frac{1}{4}\ket{21}+\frac{1}{2}\ket{22}\,,
\]
we want to predict the behavior of Alex. To do so, we need to take a more "coarse-grained" view on the results of joint measurements (observations) and stop paying attention to what Bella is doing. In other words, we need to differentiate based only on the outcomes for Alex. Looking at $\ket{T}$, we see that Alex chooses apple juice in $N/12 + N/6=N/4$ cases out of $N$. The second option appears in $N/4+N/2=3N/4$ observations. Thus, we can describe the state of Alex as
\[
\ket{A} = \frac{1}{4}\ket{1}+\frac{3}{4}\ket{2}\,.
\]
Performing the same exercise for Bella, we obtain her state
\[
\ket{B} = \frac{1}{3}\ket{1} + \frac{2}{3}\ket{2}\,.
\]
The results we obtained for Alex or Bella are called \emph{marginal distributions}. These are probabilities obtained from \emph{joint distributions} by summing over terms that do not affect the outcome we want to focus on, like in the examples above. 
\begin{exercise}
	Using the state $\ket{T_0}$ defined above, find the states of Alex and Bella.
\end{exercise}

\subsection{States Likeness}
If the behavior of Alex is described as the state $\ket{A_0}=\ket{1}$, while the behavior of Bella as $\ket{B_0}=\ket{2}$, we can confidently say that there is a big difference between them. There is nothing in common in their choices, except their complete predictability.

Suppose we have $\ket{A_1}=0.5\ket{1}+0.5\ket{2}$ and $\ket{B_1}=0.49\ket{1}+0.51\ket{2}$. Although these states are not the same, they do seem similar. It is useful to have some mathematical way to \emph{distinguish} such states, or, put differently, a quantitative measure of "likeness" of two states. Such a numeric function would have two inputs -- states $\ket{A}$ and $\ket{B}$ -- and it would produce a number representing how much in common the behaviors of Alex and Bell have. Given some such function, we could write using typical mathematical notation resembling a product($x*y$):
\[
\ket{A_0}\lozenge\ket{B_0}=0\quad\textrm{ and }\quad \ket{A_1}\lozenge\ket{B_1}\approx 1\,.
\]
The first equations says that there is nothing in common between Alex and Bella -- their preferences are completely different. The second equation states that the behaviors of both are nearly identical, with almost 100\% likeness\footnote{ We can measure "likeness" on the scale from 0 to 1, or from 0 to 100\%.}.

\begin{myrem}{Likeness and Correlation}
...
\[
	0 \le \ket{A}\lozenge\ket{B}\le 1\quad\textrm{ and }\quad \ket{A}\lozenge\ket{A}=1\,.
\]
We can also expect that the behavior of Alex resembles that of Bella's to the same degree as the behavior of latter resembles the behavior of the former. Symbolically: $\ket{A}\lozenge\ket{B}=\ket{B}\lozenge\ket{A}$. 

The problem of \emph{distinguishing} random behaviors (states) was discussed as early as 1923\footnote{Fisher RA. XXI.—On the Dominance Ratio. Proceedings of the Royal Society of Edinburgh. 1923;42:321-341.} by Sir Ronald Aylmer Fisher -- a British mathematical statistician, working in the field of biology and genetics. Fisher's approach to represent random states using \emph{angular coordinates} instead of more directly measurable probabilities was more advantageous for statistical calculations and was later analyzed by many researchers, including quantum physicist. For example, one of the founders of quantum information theory, William Kent Wootters, emphasized\footnote{Wootters, W.K. (2013). Optimal Information Transfer and Real-Vector-Space Quantum Theory. arXiv: Quantum Physics, 21-43.} the connection between Fisher's approach and the information which can be obtained during any experiment, not just quantum.
\begin{mybio}{Fisher Circle, Wootter Semicircle, and Bloch Sphere}
	Here we''' briefly outline the motivation behind representing states using square roots of probabilities: $\ket{A} = \sqrt{p_1}\ket{1}+\sqrt{p_2}\ket{2}$ instead of a more intuitive form $\ket{A} = p_1\ket{1}+ p_2\ket{2}$.
	
	Fisher, focusing on statistical genetics, was analyzing probability $p$ for a particular "value" of a gene to appear in a given population. In a simplest case the "value" was assumed binary so the probability for the other "value" of the gene is simply $q=1-p$. Instead of probability $p$ Fisher introduced an angle variable $\theta$, defined by $\cos\theta = 1 - 2p$. This is equivalent to noticing the similarity between the statistical and trigonometric identities: $p+q=1$ and $\sin^2\phi + \cos^2\phi=1$. Thus, introducing $p=\sin^2\frac{\theta}{2}=(1-\cos\theta)/2$, Fisher was able to simplify many of his calculations. 
	
	The change of coordinates suggested by Fisher can be illustrated geometrically in Fig. \ref{fig:fisherWootterBloch}. At first, each state written using probabilities $\ket{A} = p\ket{1}+ q\ket{2}$, can be depicted by an arrow or a point in the upper right part of the $(p,q)$ plane, as shown in Fig \ref{fig:fisherWootterBloch}(a). Since $p+q=1$, all states will belong to a straight segment connecting two "extreme" states $\ket{1}$ and $\ket{2}$. Using the angle $\theta$, defined by $p=\sin^2\frac{\theta}{2}$, each state takes the form $\ket{A} = \sin\frac{\theta}{2}\ket{1}+  \cos\frac{\theta}{2}\ket{2}$ and now corresponds to an arrow pointing to the semicircle in Fig. \ref{fig:fisherWootterBloch}(b).
	
	Construction similar to Fisher's was suggested by Wootters, with the only difference in the range of values for the angle. William Wootters considered change of coordinates $(p,q)\to(\cos^2\alpha, \sin^2\alpha)$. As the result, each state can be written as $\ket{A} = \cos\alpha\ket{1}+  \sin\alpha\ket{2}$ and represented using an arrow pointing to an arc of a circle in the top-right quadrant of Fig \ref{fig:fisherWootterBloch}(c).
	
	All three ways of representing states are equivalent and the choice is made based on convenience. Later we will learn one more powerful approach to represent states, based on \emph{operators}.  To close our discussion of geometrical representation of random states, we mention \emph{Bloch sphere} which is used to represent states of quantum systems -- known as  \emph{qubit} --  with two distinct "levels".  It is shown in Fig \ref{fig:fisherWootterBloch}(d) and can be understood as an extension of Fisher semicircle. The extension from semicircle to a sphere is needed because in quantum physics a general state of a two-level system is described by \emph{two angles} $\theta$ and $\phi$, as we will later learn.
\end{mybio}
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{fisherWootterBloch}
	\caption{Different approaches to represent states in probability space.}
	\label{fig:fisherWootterBloch}
\end{figure}

\subsection{State Multiplication}
The advantage of using square roots of probabilities when representing states becomes even more evident once we extend the mathematical operation of multiplication to states. The idea can be illustrated as follows. Consider a state $\ket{A}=\sqrt{p_1}\ket{1}+\sqrt{p_2}\ket{2}$. If we treat the "likeness" operation $\lozenge$ like a regular product, we obtain
\[
	\ket{A}\lozenge\ket{A}=p_1\ket{1}\lozenge\ket{1}+\sqrt{p_1p_2}\ket{1}\lozenge\ket{2}+
	\sqrt{p_2p_1}\ket{2}\lozenge\ket{1}+p_2\ket{2}\lozenge\ket{2}\,.	
\]
Now $\ket{1}\lozenge\ket{1}=1=\ket{2}\lozenge\ket{2}$, while states $\ket{1}$ and $\ket{2}$ are completely distinct: $\ket{1}\lozenge\ket{2}=0=\ket{2}\lozenge\ket{1}$. Therefore,
\[
\ket{A}\lozenge\ket{A}=p_1+p_2=1\,
\]
as required.

This example is just a quick preview of what can be gained by using alternative coordinates (e.g., angles $\theta$) together with \emph{linear operations} such as product. We will have a more extensive introduction into these ideas once we get to the basics of linear algebra in the following sections.


%\subsection{Schrodinger Students}
...
\begin{figure}[htbp]
	...
	\caption{Function can be viewed as a closed box with an input and an output.}
...
\begin{figure}[htbp]
	...
	\caption{Several types of functions: (1)"Usual" numeric function that computes a number for each numeric input value; (2)Function on "arrows" (or vectors), that computes an arrow for each arrow input value; (3) Function that computes a number (e.g., length) of an input arrow; (4) Function that transforms an input function into another function; see example in text. (5). Function that computes a single number for an input function. See section \ref{sec:Functional} for examples.}
	\label{fig:functionTypes}
\end{figure}

\subsection{Application Notation}
The dominant rule for writing function $f$ applied to an argument $x$ uses parentheses around the argument, like so: $f(x)$. This rule, however, is \emph{not absolute} and is abandoned as soon as one uses functions in linear algebra. For example, when an operator\footnote{The concept of an operator will be explained in section \ref{sec:operators}.} $\op{f}$ is applied to a vector $\ket{x}$, we would write $\op{f}\,\ket{x}$, without the parentheses. 
...
\section{Numberlikes}\label{sec:Numberlikes}
...
\]
However, not all vectors behave like arrows, so this suggestive notation is not universal. Furthermore, to introduce the basic notation of quantum theory as early as possible, we will switch to what is known as \emph{Dirac notation} for vectors. We already met with this notation when discussing probability in section \ref{subsec:statesHiddenVariables} and now is the time to extend the range of its application.

\subsection{Dirac Notation}
Instead of placing an arrow on top of a letter, we enclose the letter between a pair of symbols as follows:
\[
\vec{a}\overset{\textrm{Dirac}}{\longrightarrow}\ket{a}\,.
\]
...
Vectors are sometimes defined as mathematical quantities with \emph{magnitude} and \emph{direction}. This implies that a meaningful notion of \emph{length} (magnitude) can be assinged to vectors. For arrows this is obvious, while for other types of vector-like quantities might be not so obvious. 
...
\]
\begin{remark}
	We can formulate this last result as a rule: \emph{"numbers can be pulled outside of scalar product."}
\end{remark}
...
Among the simplest non-trivial operators are \emph{linear operators}. As mentioned earlier in section \ref{subsec:linearity}, linear functions satisfy two conditions:
\[
...
\begin{example}
	Consider an operator that transforms the state $\ket{0}$ into a linear combination $\ket{+}=(\ket{0}+\ket{1})/\sqrt{2})$ and the state $\ket{1}$ into a linear combination $\ket{-}=(\ket{0}-\ket{1})/\sqrt{2})$. It is called \emph{Hadamard} operator and has the following matrix representation.
	\[
	\op{H} =
	\begin{pmatrix}
...
\begin{mybio}{Numbers On Steroids}
	...
	\]
	We are looking for two operations with arrows in a plane such that two successive operations do not change any arrow (the second equation). Since an action of any operator on arrow rotates it and changes the length of the latter, the operator $\op{a}$ should cancel rotation and scaling done by $\op{b}$. For example, if $\op{b}$ scales arrow by a number $b$ and rotates by an angle $\beta$, the operator $\op{a}$ scales arrows by $a=1/b$ and performs rotation by $-\beta$ -- same magnitude but in the opposite direction.
\end{mybio}
\begin{exercise}
	Show that $\op{a}$ and $\op{b}$ only rotate any arrow by an angle of $60^\circ$.
\end{exercise}

\begin{mybio}{Dirac Equation}
	Paul Dirac discovered an equation that describes electron moving at any speed $v < c$ where $c$ is the speed of light. To make his equation work, he had to perform "linearization" of an older equation from quantum mechanics -- an operation that made his equation rely on linear operators instead of numeric coefficients. In short, his equation looked like this:
	\[
		(\op{A}p + \op{B}mc)\ket{\psi}=\ket{\phi}\,.
	\]
	Here $p$ is the momentum of an electron, $m$ is its mass. The exact meaning of $\ket{\psi}$ and $\ket{\phi}$ will be explained once we get to quantum theory in section \ref{sec:QuantumState}. For now it is important to appreciate the fact that Dirac's approached worked successfully once he switched from simple numbers to operators $a\to\op{A}$ and $b\to\op{B}$.
\end{mybio}

\subsection{Super-operators*}
An idea of a function is quite general, it implies mapping one value to another, or calculating result given a certain input. Given a number, a function can yield another number, or given a vector a function can produce another vector. In the latter case we call the function an \emph{operator}.  
...
\section{Functionals}\label{sec:Functional}
...
One last step is needed to rigirously prove that both ket-vectors and bra-vectors are indeed vectors: we must investigate how their components change when we change bases. The only challenge this task presents is its tediousness\footnote{See, for instance, {\it Tensors For Inquiring Minds} by Yury Deshko.}.
...
\op{U}\circ\op{T}=\op{I}\,.
\]
The inverse of an operator, if there exists one, is denoted using the negative power: $\op{U}=\op{T}^{-1}$. Not every operator has an inverse, with $\op{0}$ ("zeroing" operator) and $\op{N}$ (normalizing operator) providing simple examples.
...
\]
If this equality holds true for \emph{any} pair of vectors $\ket{a}$ and $\ket{b}$, then we will call the operator $\op{T}$ \emph{scalar-inverse} of the operator $\op{U}$:
\[
\op{U}\,\overset{\lbrack \cdot \rbrack}{=\joinrel=}\,\op{T}^{-1}\,.
\]


All rotation operators $\op{R}_\theta$ are scalar-inverse of themselves:
\[
...
One more useful relation is generated by the scalar product. As illustrated in Figure \ref{fig:dualOperators}(c), this relation means that one operator, say $\op{T}$, achieves the same affect as the other operator (e.g., $\op{U}$), while acting on the second input of the binary scalar product operator $\lbrack\cdot\rbrack$. Symbolically we can write:
\[
\ket{a}\cdot(\op{U}\ket{b})=(\op{T}\ket{a})\cdot\ket{b}\,.
\]
If this is true for all possible vectors, then we will call the operator $\op{T}$ \emph{scalar-equivalent} to the operator $\op{U}$. 
\[
\op{U}\,\overset{\lbrack \cdot \rbrack}{=\joinrel=}\,\op{T}\,.
\]


 A conventional name for the scalar-equivalent operator is \emph{adjoint}. From the definition given above it is clear that both the original operator $\op{U}$ and its adjoint $\op{T}$ act on the same ket-vector space. The adjoint of the operator $\op{U}$ is often denoted using "\emph{dagger}"-notation:
\[
\op{T}=\op{U}^\dagger\,.
\]
...
\]
In quantum theory, where ket-vectors "store" the information of all possible measurement (\emph{state} of a quantum system), the product is used to describe a \emph{joint system}, in which two distinct parts, say $A$ and $B$, can be identified and measured by different measuring devices. The situation is similar to the one described earlier in section \ref{subsec:jointMeasurement}. 
\begin{figure}[htbp]
	\centering
...
\]
From the probability theory it is known (see section \ref{subsec:jointMeasurement}) that the numbers $p_{ij}=a_ib_j$ give the probability to measure the value $\ket{i}$ in the experiment on the system $A$, while getting the value $\ket{j}$ in the measurement performed on $B$.
...
\begin{exercise}
	Given an operator $\op{T}=\ketbra{c}{d}$, show that its adjoint is $\op{T}^\dagger=\ketbra{d}{c}$\,.
	Thus, there is a simple rule:
	\[
	\left(\ketbra{c}{d}\right)^\dagger = \ketbra{d}{c}\,.
	\]
\end{exercise}
From the previous exercise follows that every projector operator $\op{\Pi}=\ketbra{a}{a}$ is its own adjoint:
\[
\op{\Pi}^\dagger=\op{\Pi}\,.
\]
Such operators are called \emph{self-adjoint}, they play an important role in quantum theory, representing measurements of various physical variables, such as position, momentum, energy, or angular momentum and others.
\begin{mybio}{Hermitian Operators and Polynomials}
	In quantum theory self-adjoint operators are called \emph{Hermitian operators}, after French mathematician Charles Hermite (1822 – 1901). Thus, in quantum theory, \emph{Hermitian operator} has the property
	\[
	\op{T}=\op{T}^\dagger\,.
	\]
	
	
	Among several mathematical concepts named after Hermite, there are \emph{Hermite polynomials}, which satisfy a simple recurrent relation:
	\[
	\textrm{H}_{n+1}=2x\textrm{H}_{n}-2n\textrm{H}_{n-1}\,,
	\]
	with the first two polynomials being $H_0=1$, $H_1=2x$. Hermite polynomials appear in the analysis of \emph{quantum harmonic oscillator.}
	...
	%\op{D}=2x-\partial_x
	%\]
	%The first term raises the degree of a polynomial, while the second lowers it.
	%\[
	%H_n = \op{D}^n\,1\,.
	%\]
\end{mybio}
...
\begin{remark}
	Conjugation rule:
	\[
	\ketbra{a}{b}\overset{\dagger}{\longleftrightarrow}\ketbra{b}{a}\,.
	\]
\end{remark}

\subsection{Tensor Product of Operators}
The idea of "bundling" can be extended to operators. Thus, given the operators $\op{A}$ and $\op{B}$, we can write their tensor product:
\[
\op{C}=\op{A}\otimes\op{B}\,.
\]
Such an operator expresses an action (measurement or interaction) on two parts of a \emph{composite} quantum system, e.g. two atoms in a molecule or two "beams of light" in a single experiment. To illustrate further, imagine we are studying two atoms, described by the state vectors $\ket{a}$  and $\ket{b}$, respectively. As explained above, the system of two independent atoms as one whole can be described using tensor product:
\[
\ket{\psi}=\ket{a}\otimes\ket{b}\,.
\]
Now if we perform some operation $\op{A}$ on the first atom, and some operation $\op{B}$ on the second atom in parallel, we can express this as follows:
\[
\op{C}\ket{\psi}=\left(\op{A}\otimes\op{B}\right)\left(\ket{a}\otimes\ket{b}\right)\,.
\] 
...
\[
\op{a}\quad\to\quad \op{A}\ket{a}\,,
\]
and
\[
\op{b}\quad\to\quad \op{B}\ket{b}\,.
\]
Therefore, the final state of the composite system becomes:
\[
\ket{\phi} = \left(\op{A}\ket{a}\right)\otimes\left(\op{B}\ket{b}\right)\,.
\]
We can summarize it all as a rule:
\[
\op{A}\otimes\op{B}\left(\ket{a}\otimes\ket{b}\right)= \op{A}\ket{a}\otimes\op{B}\ket{b}\,.
\]
...
\emph{If} trigonometric functions $\cos$ and $\sin$ acted as "basis", then this expression could be viewed as a vector:
\[
\ket{f}=2\ket{\cos}+\frac{1}{3}\ket{\sin}\,.
\]
...
\[
\ket{f} = c_1\ket{b_1}+c_2\ket{b_2}+\ldots+c_n\ket{b_n}\,.
\]
 ...
 f_x = (1+x) e^{-x^3\sin x}\,,
 \]
 built from product and composition of polynomials, trigonometric, and exponential functions. The graph of $f_x$ is plotted in Figure \ref{fig:functionApproximation} using open circles. Two different ways to approximate this funciton in the range $\lbrack -2, 2\rbrack$ is demonstrated in parts (a) and (b). Let's take a closer look.
 
 \begin{figure}[htbp]
 	\centering
 	\includegraphics[scale=1.0]{functionApproximation}
 	\caption{A function $f_x=(1+x)e^{-x^3\sin x}$ can approximated using polynomials or trigonometric functions.}
 	\label{fig:functionApproximation}
 \end{figure}

\subsection{Taylor Series}
In a finite range of interest, it possible to represent a function using polynomials. The function $f_x$ defined above, behaves almost like the following polynomial of the 13th degree:
\begin{eqnarray*}
	P_x & = 1 + x + 0.107x^2 + 0.156x^3-1.565x^4-1.669x^5+\\&+1.27x^6+ 
	1.362x^7-0.444x^8-0.484x^9+0.74x^{10}+\\
	&+0.082x^{11}-0.005x^{12}-0.005x^{13}\,.
\end{eqnarray*}
Polynomial approximation works well for all functions which are "well-behaved" (smooth and not blowing up to infinity) in a finite range of interest. The smaller the range, the simpler the required polynomial becomes. This idea is illustrated in Figure \ref{fig:polynomialApproximation}. At the highest zoom level any function looks like a piece of straight line, described by a first degree polynomial $a_0+a_1x$. Expanding the range (zooming out), the function first starts to resemble a parabola, then cubic, and so on.
 \begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{polynomialApproximation}
	\caption{Many "well-behaved" functions can be described using polynomials when "zoomed in" on a finite range. The smaller the range of interest -- the lower the degree of the polynomial needed for accurate fit.}
	\label{fig:polynomialApproximation}
\end{figure}

It can be shown that given a function $f_x$, it can be approximated around some point of interest $x=a$ using a simple series:
\[
f_x \approx c_0 + c_1(x-a)+c_2(x-a)^2+\ldots+ c_n(x-a)^n 
\]
or more compactly
\[
f_x \approx \int c_i(x-a)^i\,,\quad i=\overline{1, n}\,.
\]
This series, first introduced by an English mathematician Brook Taylor in 1715, is called \emph{Taylor series}.

As the result, the information about a function $f$ becomes "encoded" in a list of coefficients of the polynomial:
\[
f\quad\leftrightarrow\quad (c_0, c_1, c_2, \ldots, c_n)\,.
\]
The function can then be written similar to a vector with components $c_i$ and some basis $\ket{u_i}$ (polynomials in this case):
\[
\ket{f} = \int c_i\ket{u_i}\,,\quad i=\overline{1, n}\,.
\]
Like in the case of arrow-vectors, the basis is not unique. Let's examine another way to represent a function using $\sin$ and $\cos$.

\begin{mybio}{Analytic Functions}
	In order to absolutely precisely represent a well-behaved function $f$ using polynomials, one needs an \emph{infinite} number of coefficients:
	\[
	f\quad\leftrightarrow\quad (c_0, c_1, c_2, \ldots, c_n,\ldots)\,.
	\]
	If a function allows such representatation, it is called \emph{analytic function}. Analytic functions are of great value in mathematical physics, where they often describe very smooth physical behavior without jumps or infinities.
\end{mybio}


\subsection{Fourier Series}
As shown in Figure \ref{fig:functionApproximation}(b), the same function $f_x=(1+x)e^{-x^3\sin x}$ can be well approximated using the following series:
\begin{align*}
	F_x = & \frac{954}{1000}+\frac{571}{1000}\cos x+\frac{4}{10}\sin x+\frac{34}{1000}\cos (2x)+\frac{212}{1000}\sin(2x)-\\
	& -\frac{78}{1000}\cos (3x)-\frac{28}{1000}\sin(3x)-\frac{11}{1000}\cos (4x)-\frac{32}{1000}\sin (4x)\,.
\end{align*}
The similarity between $f_x$ and $F_x$ is only approximate and works for a limited range. 

A French mathematician and physicist Joseph Fourier introduced these simple trigonometric series in his 1822 work \emph{"The Analytical Theory of Heat"}. They are known as \emph{Fourier series}, and are used to represent \emph{periodic} functions as an infinite sum
\[
F_x = \int \left\lbrack a_n\cos(n x)+b_n\sin(n x)\right\rbrack \,,\quad n=\overline{0,\infty}\,.
\]
 Non-periodic functions, such as $f_x$, can be represented either only approximately, or using $\cos$ and $\sin$ with summation over \emph{non-countable/continuous} index $n$. 
 
 \begin{mybio}{Fourier Transform}
 	The operation of representing functions with either countable or continuous summation over trigonometric functions is called \emph{Fourier transform}. It is a very powerful mathematical tool, widely used in science and engineering.
 	
 	\[
 	\ket{f} =  \int \left\lbrack a_n\ket{\cos_n}+b_n\ket{\sin_n}\right\rbrack \,,\quad n=\overline{0,\infty}\,.
 	\]
 \end{mybio}

Again we have a situation when a function can be described compactly with a set of coefficients:
\[
f\quad\leftrightarrow\quad (a_0, a_1, b_1, a_2, b_2, \ldots, a_n, b_n)\,.
\]

\subsection{Hilbert Spaces}
We saw indications that functions, being "numberlikes", also have vector properties and can be expanded in certain basis. A basis can be, in theory, \emph{infinite} (in numerical practice it is always finite):
\[
\ket{f} = \int c_i\ket{u_i}\,,\quad i=\overline{1, \infty}\,.
\]
In physical applications, including quantum theory, the coefficients and the basis have  well-defined meaning. For example, coefficients can represent \emph{amplitudes} of oscillations of a body, or powers in certain \emph{"communication channels"}, or some other physical property. These interpretations suggest, firstly, that each coefficient is finite, and secondly that there is some \emph{limit on their total sum}. For example, if the coefficients represent the amplitudes of oscillations with various frequencies, then the square of a coefficient $c_i$ would be related to the energy of an oscillation (See section YYY) and we would have
\[
\int |c_i|^2  = N < \infty\,
\]
as a mathematical statement that the total energy of an oscillating system is finite.

The finite sum of the coefficients (or their squares) has an important consequence: it can be used to define a \emph{magnitude} (a "length") of a function as equal to the sum. This leads to a possibility of a \emph{scalar product} for a pair of functions and everything that the scalar product enables (e.g. dual spaces).

Although not all functions have the nice properties described above, a great number of functions used in quantum theory do. These functions, taken as one whole set, form a \emph{space} with length and scalar product operation defined. A special feature of this space is that, in general, basis must be \emph{infinite}. Of course, in practice, and even in many special theoretical problems, a finite number of spaces is used.

Vector spaces, endowed with scalar product, but requiring infinite bases, is called \emph{Hilbert space}, in honor of a German mathematician David Hilbert. Hilbert spaces are very important  mathematical tools in quantum theory.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{circularMotion}
	\caption{Circular motion.}
	\label{fig:circularMotion}
\end{figure}
Consider a  particle moving in a circle with the radius $R$, as shown in Figure \ref{fig:circularMotion}. If we choose the center of the circle as the reference point, we can specify the position of the particle using an arrow $\ket{r}$. During motion the direction of this arrow is constantly changing, but its length $R$ remains the same.  
...
The length of the path covered by the particle during the time interval $\delta t$ can be approximated by the length of the arc  $\delta L=R\delta\theta=v\delta t$. The arrow $\delta\ket{r}$ can be written as $\delta L\ket{u}$ where $\ket{u}$ is the unit length vector pointing in the direction of motion. This unit vector can be constructed from $\ket{r}$ by first rotating the latter counter-clockwise with the operator $\op{J}$ and then scaling the result down by $R$: 
\[
\ket{u}=\frac{1}{R}\op{J}\ket{r}\,.
\]
Thus,
\[
\delta\ket{r}=R\delta\theta\frac{1}{R}\op{J}\ket{r}=\delta\theta\op{J}\ket{r}\,.
\]
The rate of change of the vector $\ket{r}$ with respect to time then becomes
\[
\frac{\delta\ket{r}}{\delta t}=\frac{\delta\theta}{\delta t}\op{J}\ket{r}\qquad\Longrightarrow\qquad
\partial_t\ket{r}=\omega\op{J}\ket{r}\,,
\]
where we introduced the angular speed $\omega=\partial_t\theta$. Finally, by applying the $\op{J}$ operator to both sides of the last equation, we can cast it into the "Schr\"odinger" form:
\[
\op{J}\partial_t\ket{r}=-\omega\ket{r}\,.
\]  
We can make one more step, multiplying both sides by the fundamental physical constant $\hbar$ -- known as \emph{reduced Planck's constant}-- to obtain an equation very similar to Schr\"odinger equation:
\[
\op{J}\hbar\partial_t\ket{r}=-\hbar\omega\ket{r}\,,\quad \left(\textrm{compare: }i\hbar \partial_t\ket{\Psi}=\op{H}\ket{\Psi}\right)\,.
\]

\subsection{Solution}
When the angular speed $\omega$ is constant, the equation of circular motion can be easily solved.  The simple solution will play an important part in quantum theory. To find it, we first recognize that, given  constant angular speed $\omega$, the angle swept by the vector $\ket{r}$ grows linearly with time: $\theta_t = \omega t$. Next, to obtain a position vector $\ket{r_t}$ we need to rotate the initial position vector $\ket{r_0}$ by the angle $\theta_t$:
\[
\ket{r_t}=\op{R}_\theta \ket{r_0}\,.
\]
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{circularMotionSolution}
	\caption{Circular motion.}
	\label{fig:circularMotionSolution}
\end{figure}
As  illustrated in Figure \ref{fig:circularMotionSolution}(a), we can split a single rotation by the angle $\theta$ into a large number of rotations by a smal angle $\phi=\delta \theta=\theta /N$, where $N$ is the number of steps. Mathematically this is expressed as follows:
\[
\op{R}_\theta = \op{R}_\phi\circ\op{R}_\phi\circ\op{R}_\phi\circ\cdots\circ\op{R}_\phi
\]
or more compactly
\[
\op{R}_\theta = \op{R}^N_\phi\,.
\]
Each small rotation step can be described by a simple operator which is close to the identity operator:
\[
\op{R}_\phi = \op{1}+\op{G}\phi\,.
\] 
To see this, recall that rotating a vector $\ket{r}$ by a small angle $\phi$ adds to it a tiny vector $\delta\ket{r}$ which perpendicular to $\ket{r}$ and has the length equal to the length of the small arc, as shown in Figure  \ref{fig:circularMotionSolution}(b)(c):
\[
\op{R}_\phi\ket{r}=\ket{r}+\phi\op{J}\ket{r}\,.
\]
Thus, the operator of small step rotation is given by
\[
\op{R}_\phi = \op{1}+\phi\op{J}\,.
\]
Since $\phi=\delta\theta=\theta/N$, we obtain the rotation operator by the finite angle $\theta$:
\[
\op{R}_\theta = \left( \op{1}+\frac{\theta\op{J}}{N}\right)^N\,.
\]
The expression on the right is the approximation to the exponential function:
\[
e^x\approx (1+\frac{x}{N})^N\,,
\]
and when $N$ grows increasingly larger, the approximation becomes more exact. Finally, substituting $\theta=\omega t$, we arrive at the solution of the circular motion problem. The position vector $\ket{r_t}$ changes in time according to the equation
\[
\ket{r_t} = e^{\op{J}\omega t}\ket{r_0}\,.
\]
