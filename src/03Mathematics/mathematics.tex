...
The question of how to \emph{distinguish} random behaviors (states) was discussed as early as 1923\footnote{Fisher RA. XXI.—On the Dominance Ratio. Proceedings of the Royal Society of Edinburgh. 1923;42:321-341.} by Sir Ronald Aylmer Fisher -- a British mathematical statistician, working in the field of biology and genetics. Fisher's approach to represent random states using \emph{angular coordinates} instead of more directly measurable probabilities was more advantageous for statistical calculations and was later analyzed by many researchers, including quantum physicist. For example, one of the founders of quantum information theory, William Kent Wootters, emphasized\footnote{Wootters, W.K. (2013). Optimal Information Transfer and Real-Vector-Space Quantum Theory. arXiv: Quantum Physics, 21-43.} the connection between Fisher's approach and the information which can be obtained during any experiment, not just quantum.
\begin{mybio}{Fisher Circle, Wootter Semicircle, and Bloch Sphere}
	Let's briefly outline the basic idea behind representing states using square roots of probability: $\ket{A} = \sqrt{p_1}\ket{1}+\sqrt{p_2}\ket{2}$ instead of more intuitive $\ket{A} = p_1\ket{1}+ p_2\ket{2}$.
	
	Fisher was analyzing probability $p$ for a particular "value" of a gene to appear in a given population. In a simplest case the "value" was binary and the probability for the other "value" of the gene is simply $q=1-p$. Instead of probability $p$ Fisher introduced an angle variable $\theta$ such that $\cos\theta = 1 - 2p$. This is equivalent to noticing a similarity between the statistical and trigonometric identities: $p+q=1$ and $\sin^2\phi + \cos^2\phi=1$. Thus, introducing $p=\sin^2\frac{\theta}{2}$, Fisher was able to simplify many of his calculations. 
	
	The change of coordinates suggested by Fisher can be illustrated geometrically.
\end{mybio}

\begin{mybio}{State Multiplication}
The advantage of using square roots of probabilities become even more evident when the mathematical operation of multiplication is extended to states.
\end{mybio}
