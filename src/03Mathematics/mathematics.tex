...
First of all, space is a set of mathematical objects of similar nature. For example, all numbers, or all arrows considered as one single entity, can be viewed as space. Second, unlike a simple set, space has some \emph{structure}, some \emph{relations} between its elements. For example, there might some sense of direction, or distance, or simply continuity. Advanced spaces, used in mathematical physics, have quite a lot of structure, with distances, angles, algebraic operations, and even with operations required for calculus.
...
\begin{mybio}{Bra, Ket, and Braket}
	...
	\braket{a}{b}\,.
	\]
	The whole expression is called \emph{bracket}, while $\bra{a}$ is called \emph{bra vector}, and $\ket{b}$ is called \emph{ket vector}.
\end{mybio}
...
\]
we conclude that any bra-vector can be expanded in terms of some other bra-vector, just like ket-vectors are expanded in some basis:
\[
...
To rigirously prove that both ket-vectors and bra-vectors are indeed vectors, we must investigate how their components change when we change bases. The only challenge this task presents is its tediousness (REF TO MY BOOK).

From now on we will recognize that to every ket-vector $\ket{a}$ there corresponds a \emph{dual} bra-vector $\bra{a}$. We must keep in mind that this duality has been generated by the operation of scalar product. Once a vector space has scalar product defined, we get a dual vector space "for free." Such dual vector spaces turn out extremely useful in quantum theory, as we will soon see.
