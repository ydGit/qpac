...
\lettrine[lines=2]{\color{darkocre}W}{e are now ready to take the previously developed concepts}  and tools into the realm of quantum phenomena. Firstly, we will upgrade our understanding of \emph{system} and \emph{state} to the quantum level. Then we will see how the concepts of dynamics can be applied to quantum systems in general. Finally, we will do a detailed analysis of quantum dynamics of simplest and yet most important quantum systems: \emph{qubit} and \emph{quantum oscillator}. The latter will lead to the concepts of \emph{quantum field}. We will also discuss the core quantum concept of \emph{entanglement}.
...
In quantum theory a \emph{system} is understood more broadly than in classical physics. Take, for instance, a "two-body" system consisting of a star and a planet orbiting it. Neglecting the gravitational field, we can say that this is a two-part system which can be at any time described by a state
\[
\ket{\xi} = \ket{star}\ket{planet}\,,
\] 
where $\ket{star}$ and $\ket{planet}$ are the states of the star and the planet, respectively. In addition to orbital motion, each part of this system can be rotating, and this rotation constitutes additional degree of freedom for each system. Therefore, the state of, for example, the planet has two components:
\[
\ket{planet}=\ket{orbital}\ket{spin}\,.
\]
The orbital part of the state is described by the position and momentum of the planet: $\ket{orbital}=(x, p)$. The degree of freedom related to the spinning is called \emph{intrinsic degree of freedom} as it is not directly responsible for the (outward) movement of the planet as a whole. In quantum physics such degree of freedom can be the focus of an experiment and can be considered a quantum system of its own right.


As another example consider an hydrogen atom. Viewed as a classical system it has such parts as an electron, a positron, and an electromagnetic field. The electron, however, has an \emph{intrinsic degree of freedom} -- \emph{spin} -- which is fundamentally quantum and is unrelated to "spinning". When working with atoms we can affect both "orbital" and intrinsic states of the electron. However, the energy required to do that differs greatly for spin and for the "atom" (electron+positron+field). Thus, we can choose to manipulate spin only and treat this intrinsic degree of freedom as a quantum system in its own right.
...
\begin{mydef}{Quantum System}
	A degree of freedom of a physical system can be considered a \emph{quantum system} if that degree of freedom exhibits quantum behavior.
\end{mydef}
...
State is the fundamental concept of quantum theory. It is the source of all \emph{predictions} about the results of \emph{any} possible measurement. Classical state of a system tells us what the values of various quantities \emph{certainly are}, while quantum state tells us what the results of an arbitrary measurement \emph{will be} and \emph{with what probability}.
...
\section{Correspondence Principle}\label{sec:CorrespondencePrinciple}
...
H=\frac{\omega}{2\pi}A\,.
\]
The area $A$ is a physical quantity with the units of \emph{action}.

As shown in Figure \ref{fig:phaseSpaceQuantum}(a), areas in phase space have the smallest size limited by the elementary quantum of action $h$ -- known as Planck constant. The quantization of action and, consequently, the quantization of phase-space area, has two important implications for harmonic oscillator.
\begin{figure}[htbp]
	\centering
...
For any quantum system with discrete energy states $E_0, E_1, E_2,\ldots, E_n\ldots$ the Hamiltonian operator can be written in terms of projectors:
\[
...
\begin{myrem}{Susskind-Glogower Operator}
	The operator $\op{a}_\plus$ is known as \emph{Susskind-Glogower operator}.
\end{myrem}
...
To find better operators for raising and lowering states of harmonic oscillator, we can taken a closer look at the qubit case. There we had $\op{\sigma}_\plus\,\ket{0}=1\ket{1}$ and $\op{\sigma}_\minus\,\ket{1}=1\ket{0}$\,. We explicitly added "1" in front of the final states, to highlight the following property of the $\op{\sigma}$-operators:
\[
...
\begin{exercise}
	...
	\]	
	(b) Evaluate $\op{a}_\minus\op{a}_\plus\,\ket{m}$.
\end{exercise}
\begin{exercise}
	(a) Show that 
	\[
...
\[
E_{mp} = \vec{e}_{mp} \op{a}_{mp} u_{mp}(\vec{r})\,.
\]
