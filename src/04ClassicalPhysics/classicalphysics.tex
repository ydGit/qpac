...
\subsection{Oscillator}\label{sec:Oscillator}
...
\subsection{Modes}
Let's use a simple system of two coupled oscillators to explore an important concept of \emph{modes}. The system is illustrated in Figure (\ref{fig:coupledSystem})(a). To make the analysis simpler, we will assume identical masses and spring constants. Using the qoordinates $q_1$ and $q_2$ we can write the total energy of such a system as follows:
\[
E = E_1 + E_2 + E_i\,,
\]
where 
\[
E_1 = \frac{k q_1^2}{2}+\frac{m v_1^2}{2}\textrm{ and } E_2 = \frac{k q_2^2}{2}+\frac{m v_2^2}{2}
\]
are the energies of the "oscillators" without the middle coupling spring. Both $E_1$ and $E_2$ have potential energy of stretched spring and the kinetic energy of a moving body. The term $E_i$ represents \emph{interaction} or coupling between the oscillators, and equals to the potential energy of the stretched middle spring:
\[
...
\]
Now if we look at this problem from a different point of view, using generalized coordiates 
\[
Q_1 = q_1-q_2\quad\textrm{ and }\quad Q_2 = q_1 + q_2
\]
...
\[
U_1 = v_1-v_2\quad\textrm{ and }\quad U_2 = v_1 + v_2
\]
and, consequently
\[
...
\[
E_p = \frac{k}{2}\left\lbrack q_1^2 + q_2^2 + (q_1-q_2)^2\right\rbrack
\]
when expressed in terms of the new qoordinates $Q_i$ becomes:
\[
...
\[
E_k = \frac{m}{2}\left\lbrack v_1^2 + v_2^2 \right\rbrack
\]
is transformed into 
\[
E_k = \frac{m/2}{2}\left\lbrack U_1^2 + U_2^2 \right\rbrack\,.
\]
...
\[
\omega_2 = \sqrt{\frac{k_1}{M_1}} = \omega\,,
\]
where $\omega = \sqrt{k/m}$ is the oscillation frequency of the original, unconnected oscillators.

Oscillations with frequencies $\omega_1$ and $\omega_2$ represent distinct \emph{types of motion} or \emph{modes of oscillations}. To understand what they look like in terms of motion of the original bodies, imagine we excite only $Q_1$-type oscillation, with $Q_2$ remaining constant in time. In other words, we assume
\[
Q_1 = A_1\cos\omega_1 t\quad\textrm{and }\quad Q_2 = C\,.
\]
...
The second type of motion corresponds to $Q_1=C$, and $Q_2 = A_2\cos\omega_2 t$. The constant $Q_1$ means that the distance between the bodies is fixed and they both move as one whole, with the total mass $M=2m$. This "whole" system is pushed and pulled by two springs, which combine their forces, effectively acting as the spring with the constant $2k$. This results in the same oscillation frequency.

In general, both modes can be excited and corresponding qoordinates will be changing in time as
\[
...
\]
which is simply a long sum of oscillations with various frequencies. Thus, in general, the motion of each individual body can not be characterized by any signle frequency $\Omega$.
...
Physical systems of interest can be mechanical, like the ones given in Section \ref{sec:System}, or  "non-mechanical" like electromagnetic field or even gravitational field. One can also explore dynamics of quantum systems.
...
\subsection{Phase Space}\label{subsec:PhaseSpace}
...
The \emph{field} concept is extremely important for modern physics, both classical and quantum. Without proper understanding of fields, it is impossible to appreciate the concept of a particle in current theories.

In order to understand fields, we will consider their mathematical aspect first, starting with the simplest type -- \emph{scalar field}.
Then we will see how the most fundamental physical systems can be described within the framework of fields.

\subsection{Scalar Field}
Imagine we want to describe how temperature varies inside a room, as shown in Figure \ref{fig:TemperatureRoom}. For simplicity, we assume we are interested in the area near the wall with a heater and a window. In this case at each point specified by a pair of coordinates $(x,y)$ we can assign a measured value of the temperature $T$. This results in a map
\[
(x, y)\quad\leftrightarrow\quad T
\]
or in a function $T(x,y)$. Since temperature is a simple number (a scalar), the function $T(x,y)$ is called \emph{scalar field}.

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{TemperatureRoom}
	\caption{Temperature inside a room can be described using \emph{scalar field} $T(x,y)$.}
	\label{fig:TemperatureRoom}
\end{figure}

\subsection{Vector Field}
Next, suppose we study the motion of air in the vicinity of some surface. We can specify how fast and in what direction each "particle of air" or a tiny volume of air is moving at every point of interest, as shown in Figure \ref{fig:vectorField}. This way we obtain a map
\[
(x, y)\quad\leftrightarrow\quad \vec{v}
\]
or a \emph{vector-valued function} $\vec{v}(x,y)$. This is a typical example of a \emph{vector field}.
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{vectorField}
	\caption{The state of flow of air or water can be described using \emph{vector field} $\vec{v}(x,y)$ which assigns a velocity to each point.}
	\label{fig:vectorField}
\end{figure}

\subsection{Tensor and Spinor Fields*}
The examples of scalar and vector fields can be generalized to include any kind of fields. The core idea is that we are dealing with an \emph{extended system}, some "object occupying region of space" and we want to describe its properties at each point. This way we establish a map
\[
\textrm{point} \quad \leftrightarrow \quad \textrm{value}\,.
\]
...
For EMF one needs \emph{tensors} to describe the properties correctly. In other words, EMF is mathematically described as a \emph{tensor field}.

For EPF one needs \emph{spinors} -- special "vector-like" objects that capture the properties of electrons related to spin. Put differently, EPF is mathematically described as a \emph{spinor field}.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=1.0]{fieldMechanicalModel}
	\caption{Mechanical model of coupled oscillators can be used to describe physical fields, such as electromagnetic field or electron-positron field.}
	\label{fig:fieldMechanicalModel}
\end{figure}
